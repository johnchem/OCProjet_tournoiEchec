...
def test_update_tournament(tournament, db_file, parameter_modifier, text = ""):
	...
	_, modified_tournament.id = save_tournament_data(modified_tournament.serialize(), db_file)
	...
	#contrôle
	assert imported_tournament == modified_tournament, print(f'{imported_tournament} {modified_tournament}')
	for key, new_value in parameter_modifier:
...
if __name__ == "__main__":
	...
	test_update_tournament(tournament_1, [("name", "tutu"),
										  ("date", date),
										  ("description", text)])
